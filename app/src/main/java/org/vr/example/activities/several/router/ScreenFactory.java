...
import org.vr.example.activities.several.screen.home.DaggerHomeScreenComponent;
import org.vr.example.activities.several.screen.home.HomeScreenComponent;
import org.vr.example.activities.several.screen.splash.DaggerSplashScreenComponent;
import org.vr.example.activities.several.screen.splash.SplashScreenComponent;
import org.vr.example.bootstrap.BootstrapMeta;
import org.vr.example.common.screens.DaggerRootScreenMeta_RootScreenComponent;
...
public class ScreenFactory extends HashMap<String, ScreenFactory.Func>{
    ...
    public ScreenFactory(@RootScreenMeta.RootScreenQualifier String rootScreenKey,
        ...
        put(homeScreenKey, params -> new HomeScreen((context, screen, deps) -> {
            HomeScreenComponent homeScreenComponent = DaggerHomeScreenComponent.builder()
    ...
    private ExampleApplicationMeta.ExampleApplicationComponent getApplicationComponent(Context context) {
        return (ExampleApplicationMeta.ExampleApplicationComponent) context.getSystemService(DiNames.APPLICATION_COMPONENT);
    }

    @NonNull
    private BootstrapMeta.BootstrapComponent getBootstrapComponent(@NonNull Context context) {
        return (BootstrapMeta.BootstrapComponent) context.getSystemService(DiNames.BOOTSTRAP_COMPONENT);
    }

    @NonNull
    private RouterComponent getRouterComponent(@NonNull Context context) {
        return (RouterComponent) context.getSystemService(DiNames.ROUTER_COMPONENT);
    }

    @NonNull
    private RootScreenMeta.RootScreenComponent getRootScreenComponent(@NonNull Context context) {
        return (RootScreenMeta.RootScreenComponent) context.getSystemService(DiNames.ROOT_SCREEN_COMPONENT);
    }
